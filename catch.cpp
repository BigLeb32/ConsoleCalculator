...
namespace {
        ...
        std::vector<double>::iterator last, Estimator& estimator)
    {
        ...
        std::generate_n(std::back_inserter(out), resamples, [n, first, &estimator,
           &dist, &rng] {
...
namespace Catch {
    ...
    {
        return Detail::unique_ptr<TestCaseInfo>(new TestCaseInfo(_className,
                    nameAndTags, _lineInfo));
    }
...
namespace Catch {
    ...
    void IStreamingReporter::listReporters(std::vector<ReporterDescription> const&
        descriptions, IConfig const& config)
    {
        ...
        IConfig const& config)
    {
        ...
        for (auto const& test : tests) {
            ...
            Catch::cout() << TextFlow::Column(testCaseInfo.name).initialIndent(2).indent(
                    4) << '\n';
            if (config.verbosity() >= Verbosity::High) {
                Catch::cout() << TextFlow::Column(Catch::Detail::stringify(
                            testCaseInfo.lineInfo)).indent(4) << std::endl;
            }
            if (!testCaseInfo.tags.empty() && config.verbosity() > Verbosity::Quiet) {
                Catch::cout() << TextFlow::Column(testCaseInfo.tagsAsString()).indent(
                        6) << '\n';
            }
        }
        ...
        if (!config.hasTestFilters()) {
            ...
            Catch::cout() << pluralise(tests.size(),
                    "matching test case") << '\n' << std::endl;
        }
    }
          ...
          public:
            ...
            void use(Colour::Code _colourCode) override
            {
                switch (_colourCode) {
                    ...
                    case Colour::White:     return setTextAttribute(FOREGROUND_GREEN |
                    ...
                    case Colour::Cyan:      return setTextAttribute(FOREGROUND_BLUE |
                                FOREGROUND_GREEN);
                    case Colour::Yellow:    return setTextAttribute(FOREGROUND_RED |
                                FOREGROUND_GREEN);
                    case Colour::Grey:      return setTextAttribute(0);

                    case Colour::LightGrey:     return setTextAttribute(FOREGROUND_INTENSITY);
                    case Colour::BrightRed:     return setTextAttribute(FOREGROUND_INTENSITY |
                                FOREGROUND_RED);
                    case Colour::BrightGreen:   return setTextAttribute(FOREGROUND_INTENSITY |
                                FOREGROUND_GREEN);
                    case Colour::BrightWhite:   return setTextAttribute(FOREGROUND_INTENSITY |
...
namespace Catch {
    ...
    void ExceptionTranslatorRegistry::registerTranslator(const IExceptionTranslator*
        translator)
    {
        m_translators.push_back(Detail::unique_ptr<const IExceptionTranslator>
            (translator));
    }
...
namespace Catch {
    ...
    struct StringStreams {
        ...
        {
            if (m_unused.empty()) {
                m_streams.push_back(Detail::unique_ptr<std::ostringstream>
...
namespace Catch {
    namespace TestCaseTracking {
        ...
        ITracker& TrackerContext::startRun()
        {
            ...
            m_rootTracker = std::make_shared<SectionTracker>(NameAndLocation("{root}"s,
        ...
        void TrackerBase::close()
        {
            ...
            switch (m_runState) {
                ...
                case ExecutingChildren:
                    ...
                    m_children.end(), [](ITrackerPtr const & t) { return t->isComplete(); }))
                    m_runState = CompletedSuccessfully;
                    break;
...
namespace Catch {
    ...
    {
        return Detail::unique_ptr<ITestInvoker>(new TestInvokerAsFunction(
                    testAsFunction));
    }
...
namespace Catch {
    namespace Matchers {
        ...
        RegexMatcher::RegexMatcher(std::string regex,
            CaseSensitive caseSensitivity): m_regex(std::move(regex)),
            m_caseSensitivity(caseSensitivity) {}
...
namespace Catch {
            ...
            if (!m_isOpen) {
                ...
                *this << RowBreak();
                ...
                for (auto const& info : m_columnInfos) {
                    headerCols += TextFlow::Column(info.name).width(static_cast<std::size_t>
                            (info.width - 2));
                    headerCols += spacer;
                }
                m_os << headerCols << '\n';
    ...
    void ConsoleReporter::benchmarkPreparing(std::string const& name)
    {
        ...
        auto nameCol = TextFlow::Column(name).width(static_cast<std::size_t>
        ...
        std::size_t indent)
    {
        ...
        if (i != std::string::npos)
            ...
            i = 0;
        stream << TextFlow::Column(_string).indent(indent + i).initialIndent(
                indent) << '\n';
    }
...
namespace Catch {
    ...
    void ListeningReporter::listReporters(std::vector<ReporterDescription> const&
        descriptions, IConfig const& config)
    {
        for (auto const& listener : m_listeners) {
